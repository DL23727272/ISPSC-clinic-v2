...
while ($data = $result->fetch_assoc()) {
    ...
    // Create PDF (longer page height, auto break on if needed)
    ...
    ';
    $courseYear = trim($data['course'] . ' - Year ' . $data['year']);

    // Build Course/Year value dynamically
    $courseYear = trim($data['course'] . ' - Year ' . $data['year']);
    ...
    <table border="1" cellpadding="3" cellspacing="0" width="100%">
        ...
        <tr>
            <td colspan="3"><b>Name:</b> '.$data['last_name'].', '.$data['first_name'].' '.$data['middle_name'].' '.$data['suffix'].'</td>
            <td><b>Age:</b> '.$data['age'].'</td>
            <td colspan="2"><b>Course/Year:</b> '.$courseYear.'</td>
            <td colspan="2"><b>Birth Sex:</b> 
                '.($data['sex']=="Male" ? "☑" : "☐").' Male &nbsp;&nbsp;
        ...
        <tr>
            ...
            <td colspan="4"><b>Contact No.:</b> '.$data['contact_no'].'</td>
        </tr>
        <tr>
            <td colspan="2"><b>Blood Type:</b> '.$data['blood_type'].'</td>
            <td colspan="3"><b>Allergy Alert:</b> '.$data['allergy_alert'].'</td>
            <td colspan="3"><b>Disability (if any):</b> '.$data['disability'].'</td>
        </tr>
    </table>
    <br>
    ';
...
$pdf->SetAutoPageBreak(TRUE, 15);
 ...
 // Add to PDF
    ...
    // Save each file
    $filename = $exportDir . "/student_health_" . $student_id . ".pdf";
    $pdf->Output($filename, "F"); // "F" = save to file
